...
open RpcTypeExporter.ApiSpecification.ValueSerialization
...
let sampleRecordSpec = ValueSpec.Complex([
...
let ``Sample record serialized`` () =    
    ...
    let expectedValue = Value.Complex([
...
type NestedRecordWithList = 
  { str: string
    ...
    record: SampleRecord 
    list: int list}
let nestedRecordWithListSpec = ValueSpec.Complex([
...
let ``Nested record with list serialized`` () = 
    ...
    let expectedValue = Value.Complex([
        ...
        { name = "record"; value = Value.Complex([
